...
if not column_exists(cursor, "users", "monthly_earned"):
    cursor.execute("ALTER TABLE users ADD COLUMN monthly_earned INTEGER DEFAULT 0")

if not column_exists(cursor, "users", "last_season_reset"):
    cursor.execute("ALTER TABLE users ADD COLUMN last_season_reset TEXT")
...
def build_progress_bar(current, required, length=15):
    if required == 0:
        return "███████████████"
    ...
    return "█" * filled + "░" * empty
def get_user_level_data(total_earned: int):
    levels = [
        ("🥉 Bronze", 0),
        ("🥈 Silver", 300),
        ("🥇 Gold", 1000),
...
async def user_profile(callback: CallbackQuery):
    ...
    (level_name, level_required), next_level = get_user_level_data(total_earned)
    ...
    if next_level:
        next_name, next_required = next_level
        remaining = next_required - total_earned
        progress = total_earned - level_required
        total_needed = next_required - level_required

        bar = build_progress_bar(progress, total_needed)

        text += (
            f"\n🎯 До {next_name} осталось: <b>{remaining} ⭐</b>\n"
            f"<code>{bar}</code>\n"
        )
    else:
...
async def check_monthly_reset():
    ...
    cursor.execute("SELECT last_season_reset FROM users LIMIT 1")
    row = cursor.fetchone()
    ...
    for i, (uid, earned) in enumerate(top3):
        bonus = rewards[i] * 10
        add_stars(uid, bonus, "season_reward", "Награда за топ месяца")

        try:
            await bot.send_message(
                uid,
                f"🏆 Вы заняли место #{i+1}!\n"
                f"🎁 Награда: {rewards[i]}$"
            )
        except:
            pass

    cursor.execute("UPDATE users SET monthly_earned = 0")
    cursor.execute("UPDATE users SET last_season_reset = ?", (current_month,))
    conn.commit()
